...
exports.init = (sequelize, optionsArg) => {
  ...
  function createBeforeHook(operation) {
    const beforeHook = function beforeHook(instance, opt) {
      ...
      if (opt.noPaperTrail || isNoPaperTrailContinuationKeySet(ns, options)) {
        if (options.debug) {
...
/**
 * Indicates whether or not the "noPaperTrail" continuation key is set.
 * @param {object} namespace Continuation local storage namespace
 * @param {object} paperTrailOptions Paper Trail configuration options
 * @returns {boolean} True if the "noPaperTrail" continuation key is set, false otherwise.
 */

function isNoPaperTrailContinuationKeySet(namespace, paperTrailOptions) {
  const continuationKey = paperTrailOptions.noPaperTrailContinuationKey;
  return namespace != null && continuationKey != null && namespace.get(continuationKey);
}